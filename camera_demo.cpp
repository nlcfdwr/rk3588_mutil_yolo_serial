...
int main(int argc, char *argv[]) {
  ...
  SerialSender serial("/dev/ttyS0", B115200); // 视你的串口设备路径而定
  if (!serial.isOpen()) {
    KAYLORDUT_LOG_ERROR("Failed to open serial port.");
  }
  ...
  ImageProcess image_process(options.width, options.height, 640,
                             options.is_track, options.fps, &serial);
  ...
  cv::resizedWindow("Video", 1920, 1080);
  static int image_count = 0;
  static int image_res_count = 0;
  TimeDuration time_duration;
  Timeout timeout(std::chrono::seconds(3000000000));
