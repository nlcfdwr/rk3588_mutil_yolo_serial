...
ImageProcess::ImageProcess(int width, int height, int target_size,
                           bool is_track, int framerate, SerialSender *serial_sender){
...
void ImageProcess::ProcessDetectionImage(
  ...
  for (int i = 0; i < od_results.count; ++i) {
    ...
    int center_x = (detect_result->box.left + detect_result->box.right) / 2;
    int center_y = (detect_result->box.top + detect_result->box.bottom) / 2;
    KAYLORDUT_LOG_INFO("{} @ ({} {} {} {}) {}",
                       coco_cls_to_name(detect_result->cls_id),
                       detect_result->box.left, detect_result->box.top,
                       detect_result->box.right, detect_result->box.bottom,
    ...
    if (serial_sender_ && serial_sender_->isOpen()) {
      std::string info = "X:" + std::to_string(center_x) +
                         ",Y:" + std::to_string(center_y) + "\n";
                        // ",ID:" + std::to_string(detect_result->cls_id) + "\n";
      serial_sender_->sendData(info);
    }                 
    cv::rectangle(
